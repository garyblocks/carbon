...
def plotTree(myTree, parentPt, nodeTxt):
	...
	cntrPt = (plotTree.xOff + (1.0 + float(numLeafs))/2.0/plotTree.totalW,
				plotTree.yOff+plotTree.jig*random.choice((-1,1)))
	# Plot child value
	...
	for key in list(secondDict):
		if type(secondDict[key]).__name__=='dict':
			...
			plotNode(secondDict[key], (plotTree.xOff, plotTree.yOff), cntrPt, leafNode, plotTree.fontSize)
			plotMidText((plotTree.xOff, plotTree.yOff+plotTree.jig*random.choice((-1,1))), cntrPt, str(key))
	plotTree.yOff = plotTree.yOff + 1.0/plotTree.totalD
